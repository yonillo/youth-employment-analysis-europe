<!-- ESTE DOCUMENTO TIENE COMO OBJETIVO FACILITAR LA BÚSQUEDA DE TEMAS PARA EL PROYECTO PERSONAL ANALIZANDO RAPIDAMENTE Y DE FORMA AUTOMÁTICA datasets DE Eurostat. PARA ELLO SOLO HAY QUE CAMBIAR EL NOMBRE DEL CÓDIGO DEL dataset QUE FIGURA DEBAJO Y COMPILAR CON UN knit EL DOCUMENTO. UNA VEZ ELEGIDO EL TEMA Y VERIFICADO, EN LA WIKI DEL CAMPUS VIRTUAL, QUE NO HA SIDO ELEGIDO PREVIAMENTE POR OTRO ALUMNO, SE ESCRIBIRÁN LOS DATOS EN LA WIKI Y, DESPUÉS, SE UTILIZARÁ EL FICHERO ModeloValidacionTemaProyectoPersonal.Rmd PARA COMPLETAR Y SUBIR AL CAMPUS VIRTUAL EL TEMA Y SER VALIDADO POR EL PROFESOR --> 
...
```{r,echo=TRUE}
eurostac_real_nuts <- read_csv("https://ctim.ulpgc.es/AEDV/data/eurostac_real_nuts.csv") %>% 
  as_tibble() 
```
...
data <- get_eurostat(dataset.name, time_format = "date") %>%
  as_tibble() %>%
  #filter(geo %in% eurostac_real_nuts$geo) %>%
